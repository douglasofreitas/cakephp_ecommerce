<?php 
/*
 * Componente para obter dados pelo site dos Cielos
 */
...
class PedidoCielo
{
	...
	private $endereco_cielo = 'https://qasecommerce.cielo.com.br/servicos/ecommwsec.do';
	...
	public function Enviar($vmPost, $transacao)
	{
		...
		$vmResposta = $this->httprequest($endereco_cielo, "mensagem=" . $vmPost);
	...
	{
		$msg = $this->XMLHeader() . "\n" .
			   '<requisicao-autorizacao-portador id="' . md5(date("YmdHisu")) . '" versao ="1.1.0">' . "\n"
			   		. '<tid>' . $this->tid . '</tid>' . "\n   "
			        . $this->XMLDadosEc() . "\n   " 
			        . $this->XMLDadosCartao() . "\n   "
			        . $this->XMLDadosPedido() . "\n   "
	...
	{
		$msg = $this->XMLHeader() . "\n" .
			 '<requisicao-autorizacao-tid id="' . md5(date("YmdHisu")) . '" versao="1.1.0">' . "\n  "
			 	. '<tid>' . $this->tid . '</tid>' . "\n  "
			 	. $this->XMLDadosEc() . "\n" .
