...
<p>
    Aviso: A mala é enviada somente para os clientes que possuem marcado a opção de receber notícias da Home Sample. Os demais clientes não são exibidos na lista abaixo.
</p>

<?php

echo $this->Form->create('User', array('url' => array('controller' => 'users', 'action' =>'mala_direta'), 'class' => 'formulario', 'id' => 'formvalidade'));
echo $this->Form->hidden('condition', array( 'value' => json_encode($condition)));
echo '<strong>Assunto *</strong>';
echo '<br/>';
echo $this->Form->input('assunto', array('label' => false, 'style' => 'width: 785px;margin-left: 0px;'));
echo '<br/><br/>';
echo '<strong>Mensagem*</strong>';
echo '<br/>';
echo $this->Form->textarea('mensagem', array('style' => 'width:776px;height: 400px;', 'value' => $template));
echo '<br/>';
